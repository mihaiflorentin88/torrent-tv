...
export function BrowserPlayer({ active, onClose, onStateChanged, onAdvance }: { active: ActivePlayer; onClose: () => void; onStateChanged: () => void; onAdvance: (preferences: PlaybackPreferences) => Promise<void> }) {
  ...
  return <div ref={root} tabindex={-1} class={`video ${controlsVisible ? 'controls-visible' : ''}`} role="dialog" aria-modal="true" aria-label={`Playing ${active.download.displayTitle || active.download.filePath}`}>
    ...
    <div class="player-chrome">
      <div class="player-heading"><strong>{active.download.displayTitle || active.download.filePath}</strong><span>{[active.download.trackerName, active.download.playbackMode === 'progressive' ? 'Streaming while downloading' : 'Downloaded file'].filter(Boolean).join(' · ')}</span></div>
...
export function App() {
  ...
  const [preparing, setPreparing] = useState<{ trackerName: string; label: string; canceled?: boolean; error?: string; settingsHint?: boolean } | null>(null);
  ...
  const beginPrepare = (trackerName: string, label: string) => { prepareAbort.current?.abort(); const controller = new AbortController(); prepareAbort.current = controller; setPreparing({ trackerName, label }); return controller };
