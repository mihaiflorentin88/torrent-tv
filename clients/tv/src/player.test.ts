...
describe('playerAction LG back and Tizen media keys', () => {
  it('maps the LG webOS back keycode and preserves the Tizen media codes', () => {
    expect(playerAction('', 461)).toBe('back');
    expect(playerAction('', 415)).toBe('play');
    expect(playerAction('', 19)).toBe('pause');
    expect(playerAction('', 413)).toBe('stop');
    expect(playerAction('', 412)).toBe('rewind');
    expect(playerAction('', 417)).toBe('fast-forward');
    expect(playerAction('', 10232)).toBe('previous');
    expect(playerAction('', 10233)).toBe('next');
    expect(playerAction('', 10252)).toBe('play-pause');
  });
});
