...
The application layer declares a tracker-neutral `Tracker` port, registry, and capability descriptions. Concrete adapters implement `filelist` (private tracker with .torrent downloads and basic auth) and `piratebay` (public tracker with separate website and apibay API URLs, yielding magnet URIs). Only explicit submitted searches and background event jobs call upstream trackers. Submitted searches are persistent asynchronous jobs: the HTTP request returns cached matches immediately, the worker permanently merges eventual results into SQLite, queues expansion for each canonical title, and publishes an SSE completion event. Successful title expansion is suppressed for one hour. All ordinary navigation, filters, pagination, title details, settings, jobs, and library reads use SQLite only. FileList requests are serialized, throttled, and bounded to 140 per rolling hour; Pirate Bay requests are similarly bounded and normalized.

Hourly latest sync and weekly/manual rebuild only upsert: old observations are never deleted. Bounded catalog windows limit latest sync to 100 releases per provider per category; rebuild sync refreshes each enabled category's window. Zero-seeder rows remain durable but are excluded from discovery.
