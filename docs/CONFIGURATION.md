...
- **FileList**: `fileListEnabled` (boolean, default `true`), `fileListUrl` (default `https://filelist.io`), username, and passkey. Never enter the account password. Disabling FileList excludes it from catalog discovery and new preparation while preserving all existing FileList downloads and their playback.
- **The Pirate Bay**: `pirateBayEnabled` (boolean, default `false`), `pirateBayWebsiteUrl` (default `https://thepiratebay.org`), and `pirateBayApiUrl` (default `https://apibay.org`). Pirate Bay releases are public and acquired via magnet links.
- **Download engine**: `downloadEngine` (`native` [default] or `qbittorrent`) — the acquisition engine used for new downloads, applied when the process starts. Both engines coexist: existing downloads remain on the engine that created them, and the settings response reports `engineRunning`, the acquisition engine the running process started with. The native engine runs two isolated torrent clients: torrents whose info dictionary carries the private flag use a tracker-only client (no DHT, no peer exchange), while everything else — including magnets from The Pirate Bay, the only source authorized for public discovery — uses a second client with DHT and peer exchange enabled. `torrentPublicPeerPort` (default `0`, OS-assigned; requires restart) sets the public client's listen port; the private client keeps `torrentPeerPort`. If `qbittorrent` is selected, version 4.5.0 or newer is required to prepare new magnet releases; older versions reject with 409 Conflict (`ErrMagnetUnsupported`). Selecting qBittorrent takes the qBittorrent Web UI URL, username, and password in the same Settings fields.
