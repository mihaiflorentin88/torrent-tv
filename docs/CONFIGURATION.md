...
- **Download engine**: `downloadEngine` (`native` [default] or `qbittorrent`) — the acquisition engine used for new downloads, applied when the process starts. Both engines coexist: existing downloads remain on the engine that created them, and the settings response reports `engineRunning`, the acquisition engine the running process started with. The native engine resolves magnets using tracker-based peer discovery without DHT. If `qbittorrent` is selected, version 4.5.0 or newer is required to prepare new magnet releases; older versions reject with 409 Conflict (`ErrMagnetUnsupported`). Selecting qBittorrent takes the qBittorrent Web UI URL, username, and password in the same Settings fields.
