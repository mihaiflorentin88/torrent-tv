...
The webOS client is one IPK spanning webOS TV 4.0 (2018) through the current platform, built from the same screens and focus behavior as the Tizen client. Install it through Developer Mode (see [WEBOS.md](WEBOS.md)); the fixed application ID makes a replacement update drop onto the existing install. First launch works like the Samsung client: discovery lists validated servers on the local network, or choose **Manual address** and the system keyboard opens when you press OK on the address field. The remote's Back key behaves like the Tizen Back key, and playback uses the same controls, audio menu, subtitle overlay, delay, and aspect options. Physical-LG behavior — audible audio switching, native subtitle-track presence, and long-press Back — is still being verified on named hardware; current status lives in [WEBOS-VERIFICATION.md](WEBOS-VERIFICATION.md).
...
- **Playback fails after download:** Tizen and webOS are direct play. The server transcodes browser-hostile audio to AAC stereo, but the video codec must still be supported by the player; choose another video codec when necessary.
