...
- **Honest playback**: playback reflects actual engine capabilities. Multi-audio switching operates through standard HTML track collections; absent audio tracks report `Audio (0)` rather than synthetic entries; inband native subtitle delays that cannot be shifted without cue access surface a visible limitation message. The shared `AVPlayListener` interface gained an optional `ontrackschanged` callback beyond the original plan's listener listing, justified by delayed track arrival on webOS hardware where media element track collections populate asynchronously after prepare.
