...
### Interface Notes: AVPlayListener Extensions

- **`ontrackschanged`**: The shared `AVPlayListener` interface includes an optional `ontrackschanged?: () => void` callback beyond the plan's initial listener listing. This is justified by delayed track arrival on webOS hardware, where media element track collections (`audioTracks` and `textTracks`) can populate asynchronously after the initial prepare/metadata sequence, enabling the shared player to refresh its track inventory exactly once without manual polling or desync.
...
| **HW-03** | Virtual keyboard visibility event & transient wiggle | 1. Open Setup → Manual address.<br>2. Press OK on the address input.<br>3. Observe LG system keyboard appearance and any keyboard-OPEN transient visibility wiggle.<br>4. Dismiss via Done or Back. | LG system virtual keyboard slides into view; `keyboardStateChange` event fires; any transient visibility wiggle during keyboard opening settles cleanly without route exit; dismissal blurs input and restores green focus ring to Connect button without route exit. | **PENDING** |
...
| **HW-05** | OS browser handoff & refusal honesty | 1. Open Projects catalog.<br>2. Click an external project link card.<br>3. Verify behavior on normal launch vs when `webOSDev.launch` triggers `onSuccess` with `returnValue: false`. | On standard launch success (`returnValue !== false`), webOS launches `com.webos.app.browser` over the TV app displaying the target URL; on systems or restricted accounts where webOS invokes `onSuccess` with `returnValue: false`, the client honestly interprets this as a launch refusal, keeping the address visible on the card with refusal guidance rather than faking success. Pressing TV Back returns to Torrent TV with document state intact. | **PENDING** |
